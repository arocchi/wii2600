...
void EventHandler::handleWiiJoyAxisEvents( const int joyIndex, const int wiiJoyIndex )
{
    u32 held = WPAD_ButtonsHeld( wiiJoyIndex );
    u32 gcHeld = PAD_ButtonsHeld( wiiJoyIndex );

    expansion_t exp;
	WPAD_Expansion( wiiJoyIndex, &exp );
    bool isClassic = ( exp.type == WPAD_EXP_CLASSIC );

    float expX = wii_exp_analog_val( &exp, true );
    float expY = wii_exp_analog_val( &exp, false );
    s8 gcX = PAD_StickX( wiiJoyIndex );
    s8 gcY = PAD_StickY( wiiJoyIndex );

    int deadzone = Joystick::deadzone();

    if( held & ( WII_BUTTON_ATARI_UP | ( isClassic ? WII_CLASSIC_ATARI_UP : 0 ) ) || 
        gcHeld & GC_BUTTON_ATARI_UP || 
        wii_analog_up( expY, gcY ) )
    {
        handleJoyAxisEvent( joyIndex, 1,  -( deadzone + 1 ) );
    }
    else if( held & WII_BUTTON_ATARI_DOWN || gcHeld & GC_BUTTON_ATARI_DOWN || 
    ...
    {
        handleJoyAxisEvent( joyIndex, 1, 0 );
    }

    if( held & ( WII_BUTTON_ATARI_LEFT | ( isClassic ? WII_CLASSIC_ATARI_LEFT : 0 ) ) || 
        gcHeld & GC_BUTTON_ATARI_LEFT ||
        wii_analog_left( expX, gcX ) )
    {
        handleJoyAxisEvent( joyIndex, 0,  -( deadzone + 1 )  );
    }
    else if( held & WII_BUTTON_ATARI_RIGHT || gcHeld & GC_BUTTON_ATARI_RIGHT ||
    ...
    {
        handleJoyAxisEvent( joyIndex, 0,  0 );
    }

    if( held & ( WII_BUTTON_ATARI_FIRE | 
            ( isClassic ? WII_CLASSIC_ATARI_FIRE : WII_NUNCHECK_ATARI_FIRE ) ) || 
        gcHeld & GC_BUTTON_ATARI_FIRE )
    {
        handleJoyEvent( joyIndex, 0, 1 );
    }
    else
    {
        handleJoyEvent( joyIndex, 0, 0 );
    }
    
#if 0
    struct expansion_t data;
	WPAD_Expansion( joyIndex, &data ); 
	if( data.type == WPAD_EXP_CLASSIC ) 
    {        
        //vec2b_t center = data.classic.ljs.center;
        //handleJoyAxisEvent( joyIndex, 0, ( data.classic.ljs.pos.x - center.x ) * 1000  );
        //handleJoyAxisEvent( joyIndex, 1, ( ( data.classic.ljs.pos.y - center.y ) * 1000 ) * -1  );
 
	    s32 deviceScale = 32767 - (-32768);
	    s32 readScale = data.classic.ljs.max.x - data.classic.ljs.min.x;

        int x = data.classic.ljs.pos.x;
        if( x < data.classic.ljs.min.x )
        {
            x = data.classic.ljs.min.x;
        }
        else if( x > data.classic.ljs.max.x )
        {
            x = data.classic.ljs.max.x;
        }

		s32 val = ((x - data.classic.ljs.min.x) * 
            ((float)deviceScale / readScale)) + (-32768);
        myEvent->set( Event::SALeftAxis0Value, val );		

        //myEvent->set( Event::SALeftAxis0Value, ( (  + 4 ) - center.x ) * 1000 );		
	}   
#endif 
}


void EventHandler::handleWiiJoyHatEvents( const int joyIndex )
{
#if 0
    bool centered = true;
    ...
    {
        handleJoyHatEvent(joyIndex, 0, kJHatUp);
        centered = false;
    }
    if( held & WII_BUTTON_ATARI_DOWN || gcHeld & GC_BUTTON_ATARI_DOWN )
    {
        handleJoyHatEvent(joyIndex, 0, kJHatDown);
        centered = false;
    }
    if( held & WII_BUTTON_ATARI_LEFT || gcHeld & GC_BUTTON_ATARI_LEFT )
    {
        handleJoyHatEvent(joyIndex, 0, kJHatLeft);
        centered = false;
    }
    if( held & WII_BUTTON_ATARI_RIGHT || gcHeld & GC_BUTTON_ATARI_RIGHT )
    {
        handleJoyHatEvent(joyIndex, 0, kJHatRight);
        centered = false;
    }

    if( centered )
    {
        handleJoyHatEvent(joyIndex, joyIndex, kJHatCentered);
    }
#endif
}

// Method to handle the majority of Wii input events. Right now these are
// tied directly to the corresponding key (simulates a key press). 
//
// TODO: Find a better way to do this so that key remapping won't affect
// these.
bool EventHandler::handleWiiEvents()
{
    ...
    static s16 delay_frames = -1;
    static s16 delay_factor = -1;
    ...
    static Event::Type wii_color_mode = Event::ConsoleColor;
    ...
    {
        wii_launcher = 0;
        handleEvent(Event::LauncherMode, 0); 
    }

    if( myState == S_EMULATE )
    {
        bool isJoystick = 
            myOSystem->console().controller( Controller::Left ).type() ==
                Controller::Joystick;
        ...
        {
            wii_select = 0;
            handleEvent(Event::ConsoleSelect, 0);
        }
        ...
        {
            wii_reset = 0;
            handleEvent(Event::ConsoleReset, 0);
        }

        if( wii_bw )
        {
            wii_bw = 0;
            handleEvent(Event::ConsoleBlackWhite, 0);
            handleEvent(Event::ConsoleColor, 0);
        }

        if( down & WII_BUTTON_ATARI_CMD_MENU || gcDown & GC_BUTTON_ATARI_CMD_MENU )
        {
            frame_delay = 10;
            enterMenuMode( S_CMDMENU );
        } 
        else if( isJoystick && ( down & WII_BUTTON_ATARI_BW || gcDown & GC_BUTTON_ATARI_BW ) )
        {
            wii_color_mode =
                wii_color_mode == Event::ConsoleColor ?
                    Event::ConsoleBlackWhite : Event::ConsoleColor;
            
            handleEvent(wii_color_mode, 1);

            wii_bw = 1;
        }
        else if( down & WII_BUTTON_HOME || gcDown & GC_BUTTON_HOME )
        {
            handleEvent(Event::LauncherMode, 1);    
            wii_launcher = 1;
        }
        else if( held & WII_BUTTON_ATARI_SELECT || gcHeld & GC_BUTTON_ATARI_SELECT )
        {
            handleEvent(Event::ConsoleSelect, 1);
            wii_select = 1;
        }
        else if( held & WII_BUTTON_ATARI_RESET || gcHeld & GC_BUTTON_ATARI_RESET )
        {
    ...
    else if( myOverlay )
    {
        ...
        {
            wii_down = 0;
            myOverlay->handleKeyEvent(SDLK_DOWN, SDLK_DOWN, 0, 0);                  
        }
        if( wii_up )
        {
            wii_up = 0;
            myOverlay->handleKeyEvent(SDLK_UP, SDLK_UP, 0, 0);                  
        }
        if( wii_left )
        {
            wii_left = 0;
            myOverlay->handleKeyEvent(SDLK_PAGEUP, SDLK_PAGEUP, 0, 0);                  
        }
        if( wii_right )
        {
            wii_right = 0;
            myOverlay->handleKeyEvent(SDLK_PAGEDOWN, SDLK_PAGEDOWN, 0, 0);                  
        }
        if( wii_enter )
        {                
            wii_enter = 0;
            myOverlay->handleKeyEvent(SDLK_RETURN, SDLK_RETURN, 0, 0);  
        }
        if( wii_cancel )
        {
            wii_cancel = 0;
            myOverlay->handleKeyEvent(SDLK_ESCAPE, SDLK_ESCAPE, 0, 0);                  
        }
        if( wii_tab )
        {
            wii_tab = 0;
            myOverlay->handleKeyEvent(SDLK_TAB, SDLK_TAB, 0, 0);                  
        }
        if( wii_shift_tab )
        {
            wii_shift_tab = 0;
            myOverlay->handleKeyEvent(SDLK_TAB, SDLK_TAB, KMOD_SHIFT, 0);                  
        }
        ...
        else
        {
            if( ( ( held & ( 
                ...
                ( ( gcHeld & ( GC_BUTTON_DOWN | GC_BUTTON_UP |
                    GC_BUTTON_LEFT | GC_BUTTON_RIGHT ) ) == 0 ) &&
                ( !wii_analog_left( expX, gcX ) &&
                  !wii_analog_right( expX, gcX ) &&
                  !wii_analog_up( expY, gcY )&&
                  !wii_analog_down( expY, gcY ) ) )
            ...
            else
            {
                if( delay_frames < 0 )
                {
                    if( held & WII_BUTTON_DOWN || 
                        gcHeld & GC_BUTTON_DOWN ||
                        wii_analog_down( expY, gcY ) ) 
                    {       
                        myOverlay->handleKeyEvent(SDLK_DOWN, SDLK_DOWN, 0, 1);  
                        wii_down = 1;
                    }
                        ...
                        wii_analog_up( expY, gcY ) ) 
                    {
                        myOverlay->handleKeyEvent(SDLK_UP, SDLK_UP, 0, 1);                
                        wii_up = 1;
                    }
                    else if( held & ( WII_BUTTON_LEFT | 
                            ( isClassic ? WII_CLASSIC_BUTTON_LEFT : 0 ) ) ||
                        gcHeld & GC_BUTTON_LEFT ||
                        wii_analog_left( expX, gcX ) ) 
                    {       
                        myOverlay->handleKeyEvent(SDLK_PAGEUP, SDLK_PAGEUP, 0, 1);  
                        wii_left = 1;
                    }
                        ...
                        wii_analog_right( expX, gcX ) )
                    {
                        myOverlay->handleKeyEvent(SDLK_PAGEDOWN, SDLK_PAGEDOWN, 0, 1);                
                        wii_right = 1;
                    }
            ...
            if( down & ( WII_BUTTON_ENTER | 
                    ( isClassic ? 
                        WII_CLASSIC_BUTTON_ENTER : WII_NUNCHUK_BUTTON_ENTER ) ) ||
                gcDown & GC_BUTTON_ENTER )
            {
                myOverlay->handleKeyEvent(SDLK_RETURN, SDLK_RETURN, 0, 1);                
                wii_enter = 1;
            }
            else if( down & ( WII_BUTTON_ESC | 
                    ( isClassic ? 
                        WII_CLASSIC_BUTTON_ESC : WII_NUNCHUK_BUTTON_ESC ) ) || 
                gcDown & GC_BUTTON_ESC )
            {
                myOverlay->handleKeyEvent(SDLK_ESCAPE, SDLK_ESCAPE, 0, 1);                
                wii_cancel = 1;
            }
            else if( down & WII_TAB_FORWARD || gcDown & GC_TAB_FORWARD )
            {
                myOverlay->handleKeyEvent(SDLK_TAB, SDLK_TAB, 0, 1);                
                wii_tab = 1;
            }
            else if( down & WII_TAB_BACKWARD || gcDown & GC_TAB_BACKWARD )
            {
                myOverlay->handleKeyEvent(SDLK_TAB, SDLK_TAB, KMOD_SHIFT, 1);                
                wii_shift_tab = 1;
            }

            if( myState == S_CMDMENU )
            {
                if( ( down & WII_BUTTON_ATARI_CMD_MENU_CLOSE || 
                      gcDown & GC_BUTTON_ATARI_CMD_MENU_CLOSE ) &&
                      frame_delay == 0 )
                {
                    leaveMenuMode();
                }
...
void EventHandler::poll(uInt32 time)
{
    ...
    while(SDL_PollEvent(&event))
    {
        ...
        case SDL_MOUSEMOTION:
            ...
            if( myState == S_EMULATE )
            {
                // Only handle mouse motion events during emulation (for now)
        ...
        case SDL_MOUSEBUTTONDOWN:
            {
                ...
                if( myState == S_EMULATE )
                {
                    // Only handle mouse button events during emulation (for now)
    ...
    {
#ifdef WII
        //handleWiiJoyHatEvents( 0 );
        //handleWiiJoyHatEvents( 1 );

        if( myOSystem->console().controller( Controller::Left ).type() 
                == Controller::Paddles )
        {
            handleWiiJoyAxisEvents( 1, 2 );
        }
        else
        {
            handleWiiJoyAxisEvents( 0, 0 );
            handleWiiJoyAxisEvents( 1, 1 );
        }
#endif
...
void EventHandler::handleEvent(Event::Type event, int state)
{
    ...
    case Event::LauncherMode:
        ...
        {
            myOSystem->sound().mute(true); // WII (bug in Stella?)
