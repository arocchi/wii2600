...
bool FrameBufferSoft::setVidMode(VideoMode& mode)
{
...
#ifdef WII
    // Force 8bpp (allows to scale w/o slowdown)
    myScreen = SDL_SetVideoMode(mode.screen_w, mode.screen_h, 8, 0);
#else
  ...
  switch(myBytesPerPixel)
  {
    case 1:  // 8-bit;
      ...
      myRenderType = myUsePhosphor ? kPhosphor_8 : kSoftZoom_8;
      break;
    case 2:  // 16-bit
    ...
    case kSoftZoom_8:
        {
            ...
            uInt16* buffer    = (uInt16*)(((uInt8*)myScreen->pixels) + myBaseOffset); 
            uInt32 bufofsY    = 0;
            uInt32 screenofsY = 0;
            ...
            for(uInt32 y = 0; y < height; ++y)
            {
                ...
                while(ystride--)
                {
                    ...
                    for(uInt32 x = 0; x < width; ++x)
                    {
                        ...
                        while(xstride--)
                        {
                            buffer[pos++] = (uInt8) myAvgPalette[v][w];
                            buffer[pos++] = (uInt8) myAvgPalette[v][w];
                        }
                    }
                    screenofsY += myPitch;
...
#ifdef WII
// Should we swap the frame buffer?
extern bool os_swap_fb;
#endif
...
void FrameBufferSoft::postFrameUpdate()
{
  if(myTiaDirty && !myInUIMode)
  {
        ...
        if( os_swap_fb )
        {
          // Swap the framebuffer (for double buffering support)
...
FBSurface* FrameBufferSoft::createSurface(int w, int h, bool isBase) const
{
  ...
  isBase = false;
    ...
    // Lame hack to copy the pallete from the base surface to the newly created
    // surface. This is necessary to support 8-bit graphics mode
    if( !isBase )
    { 
        int colors = myFormat->palette->ncolors;
        surface->format->palette->ncolors = colors;
        surface->format->palette->colors = 
            (SDL_Color*)malloc( sizeof(SDL_Color) * colors );
        memcpy( 
            surface->format->palette->colors, 
            myFormat->palette->colors, 
            sizeof(SDL_Color) * colors );
    }
#else        
  SDL_Surface* surface = isBase ? myScreen :
    ...
    case 1:
        {
            ...
            for(int y = 0; y < bbh; y++)
            {
                ...
                for(int x = 0; x < bbw; x++, mask >>= 1)
                    if(ptr & mask)
                        buffer[x] = (uInt8) myFB.myDefPalette[color];
